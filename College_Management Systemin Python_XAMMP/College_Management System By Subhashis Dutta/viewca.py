...
def add():
    if name1.get()=="" or rol1.get()=="" or unirol1.get()=="" or cs1.get()=="" or courselist.get()=="" or department1.get()=="" or sub1.get()=="" or mk1.get()=="" :
     ...
     mycursor=db.cursor()
     try:
        sql="insert into ca(name,rol,unirol,cs,course,department,sub,marks)values(%s,%s,%s,%s,%s,%s,%s,%s)"
...
def fectdata():
    ...
    db=mysql.connector.connect(host="localhost", user="root", password="", database="sd")
    mycursor = db.cursor()
    mycursor.execute("select * from ca")
...
def update():
    ...
    mycursor = db.cursor()
    try:
        sql = "update ca set rol=%s,unirol=%s,cs=%s,course=%s,department=%s,sub=%s,marks=%s where name=%s"
...
def delete1():
    ...
    mycursor = db.cursor()
    try:
        sql = "delete from ca where name=%s"
        val = (nam,)
        mycursor.execute(sql, val)
        db.commit()
...
def fectdata1():
    ...
    db = mysql.connector.connect(host="localhost", user="root", password="", database="sd")
    mycursor = db.cursor()
    mycursor.execute("select * from ca where "+str(ser1)+" LIKE '%"+str(lsearch1)+"%'")
    rows=mycursor.fetchall()
    if len(rows)!=0:
        studtab.delete(*studtab.get_children())
    for row in rows:
...
title=Label(rt,text="View CA Marks",bd=5,relief=GROOVE,font=("constantia",30,"bold"),bg="yellow",fg="black")
